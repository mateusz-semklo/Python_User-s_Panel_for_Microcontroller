...
def update():
    global id_loop,flag,speed,encoder,t,ts,xs,ys,j,fig,animate
    message = '{"settings":"n"}'
    #js=json.loads(jstring)
    #js["settings"]='n'
   ...
   #message = json.dumps(js)
    ...
    try:
        ...
        data=datax.decode(encoding='utf-8',errors='strict')    
...
button1 = Button(window, text = "Wyslij ustawienia",height=2 ,command=fun_settings)
button1.grid(row=i,column=2,rowspan=2,columnspan=2)
button5 = Button(window, text = "Start", height=2,width=10 ,command=fun_start)
button5.grid(row=i,column=3,columnspan=3,rowspan=2)

button6 = Button(window, text = "Stop",height=2,width=10 ,command=fun_stop)
...
i=i+1
button2 = Button(window,height=2, text = "Rysuj wykres", command=fun_draw)
button2.grid(row=i,column=0)
button3 = Button(window,height=2, text = "Stop wykres", command=fun_stop_draw)
